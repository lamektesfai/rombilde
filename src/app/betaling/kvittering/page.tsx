...
function erFerdig(status: JobStatus) {
  return status === "completed" || status === "failed";
}

function RombildeHeader() {
  return (
...
function EnkeltJobbKvittering({ jobId }: { jobId: string }) {
  ...
  return (
    <div className="flex w-full max-w-lg flex-col items-center gap-6 text-center">
      ...
      {!error && !job && <p className="text-ink-soft">Henter status på jobben din …</p>}
...
function OrdreKvittering({ orderId }: { orderId: string }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (!res.ok) throw new Error("Fant ikke ordren din.");
    ...
    poll();
    const interval = setInterval(() => {
      if (order && order.jobs.every((job) => erFerdig(job.status))) {
        clearInterval(interval);
        return;
      }
      poll();
  ...
  const alleFerdig = order ? order.jobs.every((job) => erFerdig(job.status)) : false;
  const antallFerdig = order ? order.jobs.filter((job) => job.status === "completed").length : 0;
  ...
  return (
    <div className="flex w-full max-w-3xl flex-col items-center gap-6 text-center">
      ...
      {!error && !order && <p className="text-ink-soft">Henter status på pakken din …</p>}
        ...
        <>
          {!alleFerdig ? (
            <>
              ...
              <p className="font-heading text-2xl font-semibold">
                Møblerer bildene dine … ({antallFerdig} av {order.jobs.length} ferdig)
              </p>
              <p className="text-sm text-ink-soft">
                Du kan trygt la denne siden være åpen — den oppdaterer seg selv.
              </p>
            ...
            <p className="font-heading text-2xl font-semibold">
              {antallFerdig} av {order.jobs.length} bilder er ferdig møblert
            </p>
          )}
          ...
          <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2">
            {order.jobs.map((job, index) => (
              <div
                key={job.id}
                className="flex flex-col gap-3 rounded-2xl border border-line bg-paper-2 p-4 text-left"
              >
                <span className="font-heading text-sm font-semibold">Bilde {index + 1}</span>
                ...
                {job.status === "completed" && job.resultImageUrl && (
                  <>
                    ...
                    <a
                      href={job.resultImageUrl}
                      download
                      className="rounded-full bg-pine px-4 py-2 text-center text-sm font-medium text-paper transition-colors hover:bg-pine-light"
                    >
                      Last ned bildet
                    </a>
                  </>
                ...
                {job.status === "failed" && (
                  <p className="text-sm text-ink-soft">
                    Dette bildet feilet dessverre. Vi tar kontakt med deg for å ordne opp.
                  </p>
                )}

                {!erFerdig(job.status) && (
                  <p className="text-sm text-ink-soft">{STATUS_TEKST[job.status]}</p>
                )}
              </div>
            ))}
          </div>
          ...
          {alleFerdig && (
            <>
              <p className="text-xs text-ink-soft">
                Vi har også sendt de ferdige bildene til e-posten du oppga.
              </p>
              <Link
                href="/last-opp"
                className="text-sm text-ink-soft underline-offset-4 hover:text-pine hover:underline"
...
function KvitteringInnhold() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-paper px-6 py-20 text-ink">
      {orderId ? (
        <OrdreKvittering orderId={orderId} />
      ) : jobId ? (
        <EnkeltJobbKvittering jobId={jobId} />
      ) : (
        <div className="flex w-full max-w-lg flex-col items-center gap-6 text-center">
          <RombildeHeader />
          <p className="rounded-xl border border-sand bg-sand-light px-4 py-3 text-sm">
            Mangler jobb-ID i lenken.
          </p>
        </div>
      )}
    </main>
